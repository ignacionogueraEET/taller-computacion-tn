...
/**
 * Configuración global de la capa lógica de la interfaz
 */
const CONFIG = {
  storageKey: 'tema',
...
// Rutas de vectores SVGs embebidas dinámicamente para evitar dependencias
...
/**
 * Comprueba las preferencias del estudiante para cargar el modo visual adecuado
 */
function inicializarTema() {
  ...
  const preferenciaSO = window.matchMedia(CONFIG.darkModeQuery).matches
    ? 'dark'
    : 'light';
  const temaFinal = temaGuardado || preferenciaSO || CONFIG.defaultTheme;
  aplicarTema(temaFinal);
}

/**
 * Modifica los atributos de accesibilidad y almacenamiento del tema seleccionado
 */
...
/**
 * Intercambia internamente los elementos gráficos del botón
 */
...
/**
 * Establece los listeners de interacción para el cambio de contraste claro/oscuro
 */
...
/**
 * Escanea la sección principal de la consigna para construir un índice de navegación
 */
function generarTabladeContenidos() {
  ...
  const encabezados = [];
  mainContent.querySelectorAll('h2').forEach((encabezado) => {
    if (!encabezado.id) {
  ...
  encabezados.forEach((enc) => {
    htmlList += `<li><a href="#${enc.id}">${enc.texto}</a></li>`;
  });
  htmlList += '</ul>';

  const detalles = tocContainer.querySelector('details');
  if (detalles) {
    detalles.innerHTML = `<summary>Tabla de contenidos</summary>${htmlList}`;
  } else {
    tocContainer.innerHTML = htmlList;
  }
}

/**
 * Transforma cadenas de texto con caracteres especiales en IDs limpios para navegación
 */
...
/**
 * Mantiene de manera dinámica el año en los derechos de la web institucional
 */
...
// Control anti-rebote (debounce) para optimizar el rendimiento al redimensionar pantallas
let timerResize;
